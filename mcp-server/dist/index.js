...
const tools = [
    ...
    {
        name: 'create_task',
        description: 'Create a new task in the ORIGIN development workflow. Used by ChatGPT Moderator to create and assign tasks to appropriate agents.',
        inputSchema: {
            type: 'object',
            properties: {
                title: {
    ...
    {
        name: 'route_task',
        description: 'Route a task to the appropriate agent based on complexity, content type, and agent capabilities. Primary tool for ChatGPT Moderator.',
        inputSchema: {
            type: 'object',
            properties: {
                taskId: {
    ...
    },
    {
        ...
        description: 'Update the status of a task as it progresses through the workflow. Used by agents to report progress.',
        inputSchema: {
            type: 'object',
            properties: {
                taskId: {
    ...
    {
        name: 'list_tasks',
        description: 'List tasks based on filters. Use to see assigned tasks, pending work, or track project progress.',
        inputSchema: {
            type: 'object',
            properties: {
                status: {
    ...
    // === ARTIFACT MANAGEMENT ===
    {
        ...
        description: 'Submit a work product (artifact) for a task. Used by agents to deliver their work for review or handoff to next agent.',
        inputSchema: {
            type: 'object',
            properties: {
                taskId: {
                ...
                content: {
                    type: 'string',
                    description: 'The actual artifact content (file path, URL, or inline content)',
                },
                requiresValidation: {
                    type: 'boolean',
    ...
    {
        name: 'list_artifacts',
        description: 'List artifacts based on filters. Use to find work products from specific agents or tasks.',
        inputSchema: {
            type: 'object',
            properties: {
                taskId: {
    ...
    // === VALIDATION WORKFLOWS ===
    {
        ...
        description: 'Request validation of an artifact. Used to invoke Grok 4 for safety checks or DeepSeek for correctness validation.',
        inputSchema: {
            type: 'object',
            properties: {
                artifactId: {
    ...
    },
    {
        ...
        description: 'Submit the result of a validation. Used by validator agents (Grok 4, DeepSeek) to report findings.',
        inputSchema: {
            type: 'object',
            properties: {
                validationId: {
    ...
    {
        name: 'send_message',
        description: 'Send a message to another agent or broadcast to all agents. Used for coordination and clarification requests.',
        inputSchema: {
            type: 'object',
            properties: {
                to: {
                    type: 'string',
                    description: 'Target agent (use "all" for broadcast) - chatgpt_moderator, gemini3_research, gemini_gem_curator, grok4_safety, deepseek_reasoning, claude_builder, or all',
                },
                type: {
                    type: 'string',
    ...
    {
        name: 'get_messages',
        description: 'Retrieve messages sent to current agent. Use to check for pending requests or notifications.',
        inputSchema: {
            type: 'object',
            properties: {
                unreadOnly: {
    ...
    {
        name: 'escalate_issue',
        description: 'Escalate an issue to the ChatGPT Moderator. Use when blocked, encountering errors, or needing guidance.',
        inputSchema: {
            type: 'object',
            properties: {
                taskId: {
    ...
    },
    {
        ...
        description: 'Resolve an escalated issue. Used by ChatGPT Moderator to provide guidance and close escalations.',
        inputSchema: {
            type: 'object',
            properties: {
                escalationId: {
    ...
    // === PROJECT STATE ===
    {
        ...
        description: 'Get current state of the ORIGIN project including active tasks, completed work, and agent statuses.',
        inputSchema: {
            type: 'object',
            properties: {
                includeCompleted: {
    ...
    },
    {
        ...
        description: 'Update current agent status (available, busy, offline). Used to coordinate workload.',
        inputSchema: {
            type: 'object',
            properties: {
                status: {
    ...
    {
        name: 'advance_phase',
        description: 'Advance the project to the next workflow phase. Used by ChatGPT Moderator when a phase is complete.',
        inputSchema: {
            type: 'object',
            properties: {
                currentPhase: {
        ...
        switch (name) {
            ...
            case 'create_task': {
                ...
                const task = coordination.createTask({
                    ...
                    createdBy: input.createdBy || 'chatgpt_moderator',
            ...
            case 'get_task': {
                const task = storage.getTask(args.taskId);
                return {
                    content: [
                        {
                            type: 'text',
            ...
            case 'get_artifact': {
                const artifact = storage.getArtifact(args.artifactId);
                return {
                    content: [
                        {
                            type: 'text',
            ...
            case 'get_validation_status': {
                const validation = storage.getValidation(args.validationId);
                return {
                    content: [
                        {
                            type: 'text',
