...
export class CoordinationManager {
  ...
  routeTask(params: {
    taskId: string;
    context?: string;
  }): { taskId: string; assignedTo: AgentType; reasoning: string } {
   ...
   */
  private getRoutingReasoning(phase: WorkflowPhase, agent: AgentType): string {
    const reasons: Record<string, string> = {
      intake_research:
        'Research and intake phase requires document analysis and information gathering',
      conversion_preparation:
        'LaTeX conversion requires lossless curation and segmentation',
      reasoning_analysis:
        'Complex reasoning requires mathematical analysis and proof validation',
      implementation: 'Implementation requires full-stack development capabilities',
      validation_safety:
        'Validation requires safety analysis and correctness verification',
  ...
  updateAgentStatus(params: {
    ...
    console.log(
      `[${new Date().toISOString()}] Agent ${params.agent} status: ${params.status}${params.details ? ` - ${params.details}` : ''}`
    );
  }
